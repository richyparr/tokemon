...
must_haves:
  ...
  key_links:
    - from: "Tokemon/Views/FloatingWindow/FloatingWindowView.swift"
      to: "Tokemon/Services/UsageMonitor.swift"
    ...
    - from: "Tokemon/TokemonApp.swift"
      to: "Tokemon/Services/FloatingWindowController.swift"
...
<task type="auto">
  ...
  <files>Tokemon/Views/FloatingWindow/FloatingWindowView.swift</files>
  <action>
...
- Tokemon being an LSUIElement app (no dock icon, doesn't terminate on window close)
...
2. Always on top: Click on another app (Terminal, Finder, etc.). Verify the floating window stays visible above other windows. Verify it does NOT activate Tokemon (no focus stealing).
...
4. Position persistence: Drag the floating window to a different corner. Quit Tokemon (Cmd+Q). Relaunch the app. Open floating window again - should appear in the saved position.
