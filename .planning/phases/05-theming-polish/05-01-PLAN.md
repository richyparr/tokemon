...
<objective>
Create the theme infrastructure for Tokemon: AppTheme enum defining three themes, ThemeColors struct for semantic color resolution, and ThemeManager @Observable for state management. Then add theme picker UI to AppearanceSettings.
...
<task type="auto">
  ...
  <action>
Create two new files in Tokemon/Utilities/:
...
Import SwiftUI and AppKit.
  ...
  <verify>
`swift build` compiles without errors. Both files exist in Tokemon/Utilities/.
  </verify>
  <done>
...
2. Tokemon/Utilities/Theme.swift exists with AppTheme, ThemeColors, ThemeManager
3. Tokemon/Utilities/Color+Hex.swift exists with hex color extensions
4. Tokemon/Views/Settings/AppearanceSettings.swift has Theme section with picker
