...
<task type="auto">
  ...
  <action>
Create a new Xcode project for a macOS app called "ClaudeMon" using SwiftUI lifecycle, targeting macOS 14.0+.
...
- Set bundle identifier to `com.claudemon.app`
- Set minimum deployment target to macOS 14.0

**ClaudeMonApp.swift (the @main entry point):**
- Import SwiftUI, MenuBarExtraAccess, SettingsAccess
- Create `ClaudeMonApp: App` with `@main`
...
Create a placeholder `SettingsView.swift` in `Views/Settings/` with just `Text("Settings")` for now (will be built in Plan 03).
  ...
  <verify>
Run `xcodebuild build -project ClaudeMon.xcodeproj -scheme ClaudeMon -destination 'platform=macOS'` -- build must succeed with zero errors. SPM dependencies must resolve.
  </verify>
  <done>
...
- Stub methods (real implementation in Plan 02):
  - `func startPolling(interval: TimeInterval = 60)` -- for now, sets up a Timer that calls `refresh()` repeatedly. Include App Nap prevention via `ProcessInfo.processInfo.beginActivity(options: [.background, .idleSystemSleepDisabled], reason: "ClaudeMon usage monitoring")`
  - `func stopPolling()`
