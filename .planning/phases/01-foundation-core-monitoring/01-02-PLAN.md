...
must_haves:
  ...
  artifacts:
    - path: "ClaudeMon/Services/OAuthClient.swift"
      provides: "HTTP client for /api/oauth/usage endpoint"
      exports: ["OAuthClient"]
    - path: "ClaudeMon/Services/TokenManager.swift"
      provides: "Keychain read, token expiry check, token refresh"
      exports: ["TokenManager"]
    - path: "ClaudeMon/Services/JSONLParser.swift"
...
- `static func fetchUsage(accessToken: String) async throws -> OAuthUsageResponse`:
  ...
  - Headers: `Authorization: Bearer {accessToken}`, `Accept: application/json`, `anthropic-beta: oauth-2025-04-20`, `User-Agent: ClaudeMon/1.0`
...
- `static func findProjectDirectories() throws -> [URL]`:
  ...
  - Return directories (each is a project like `-Users-richardparr-ClaudeMon`)
