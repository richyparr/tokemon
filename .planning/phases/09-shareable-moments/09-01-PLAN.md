...
<task type="auto">
  ...
  <files>Tokemon/Views/ShareableCard/ShareableCardView.swift</files>
  <action>
Create `Tokemon/Views/ShareableCard/ShareableCardView.swift` with a self-contained SwiftUI view for shareable usage cards.
...
- Tokemon branding: "Tokemon" text in Anthropic orange (0xc15f3c) at top-left
...
- Footer: "tokemon.app" text at bottom-right for viral marketing
...
File exists and contains struct: `grep -l "struct ShareableCardView" Tokemon/Views/ShareableCard/ShareableCardView.swift`
  </verify>
  <done>ShareableCardView.swift exists with self-contained view, solid colors only, all required data as parameters, Tokemon branding, and viral marketing URL.</done>
</task>
...
Add these methods after the existing CSV methods, with a `// MARK: - Image Rendering & Clipboard` section header.
  ...
  <verify>
Build succeeds: `cd /Users/richardparr/Tokemon && xcodebuild -scheme Tokemon -quiet build 2>&1 | tail -5`
Methods exist: `grep -E "renderToImage|copyImageToClipboard|copyViewToClipboard" Tokemon/Services/ExportManager.swift`
  </verify>
...
<success_criteria>
- ShareableCardView renders usage data with Tokemon branding and viral URL
