...
must_haves:
  truths:
    - "ShareableCardView renders usage stats with Tokemon branding"
  ...
  artifacts:
    - path: "Tokemon/Views/ShareableCard/ShareableCardView.swift"
...
human_verification:
  - test: "Visual appearance of usage card"
    expected: "Card shows Tokemon branding in orange, utilization %, optional project/tokens, tokemon.app footer"
...
|---|-------|--------|----------|
| 1 | ShareableCardView renders usage stats with Tokemon branding | VERIFIED | ShareableCardView.swift (111 lines) with "Tokemon" text at line 27, anthropicOrange color, utilizationPercentage display |
...
| 6 | Usage card includes Tokemon branding | VERIFIED | "Tokemon" text (line 27), anthropicOrange color (line 15), "tokemon.app" footer (line 74) |
...
|----------|----------|--------|---------|
| `Tokemon/Views/ShareableCard/ShareableCardView.swift` | Shareable card SwiftUI view | VERIFIED | 111 lines, contains struct ShareableCardView, branding elements, pill badges, token formatting |
| `Tokemon/Services/ExportManager.swift` | Image rendering and clipboard methods | VERIFIED | 215 lines, contains renderToImage, copyImageToClipboard, copyViewToClipboard methods |
| `Tokemon/Views/Analytics/AnalyticsDashboardView.swift` | Share Usage Card button | VERIFIED | 203 lines, contains isCopied state, Share Usage Card button, performCardCopy method |
...
| SHARE-03: Usage card includes Tokemon branding (viral marketing) | SATISFIED | None |
...
**Expected:** Card shows:
- "Tokemon" text in Anthropic orange (#c15f3c) at top-left
...
- "tokemon.app" footer at bottom-right
...
1. **ShareableCardView** - A self-contained SwiftUI view that renders usage stats with Tokemon branding, following ImageRenderer constraints (no @Environment, no gradients)
