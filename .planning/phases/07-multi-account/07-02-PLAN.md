...
must_haves:
  ...
  artifacts:
    - path: "Tokemon/Views/MenuBar/AccountSwitcherView.swift"
      provides: "Account picker dropdown in popover header"
      contains: "struct AccountSwitcherView"
    - path: "Tokemon/Views/Settings/AccountsSettings.swift"
  ...
  key_links:
    ...
    - from: "Tokemon/Views/MenuBar/AccountSwitcherView.swift"
      to: "Tokemon/Services/AccountManager.swift"
...
`swift build 2>&1 | head -20` completes without errors
  ...
  <done>
AccountManager is initialized in TokemonApp, passed via environment, and account changes trigger UsageMonitor refresh with account-specific credentials.
  </done>
</task>
...
<success_criteria>
- AccountManager integrated into TokemonApp with environment
...
- User can add a second Claude account via OAuth: run `claude login` externally, click "Check for New Accounts", Tokemon scans keychain using allKeys() and imports new accounts
</success_criteria>
