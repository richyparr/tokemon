...
struct ExportManager {
    ...
    static func exportPDF(reportView: some View, suggestedFilename: String = "ClaudeMon-Report.pdf") async -> Bool {
    ...
    ///   - suggestedFilename: Default filename for the save dialog.
    /// - Returns: true if export succeeded, false otherwise.
    static func exportCSV(from points: [UsageDataPoint], suggestedFilename: String = "claudemon-usage.csv") async -> Bool {
        // Critical for LSUIElement apps: activate before showing save panel
        NSApp.activate(ignoringOtherApps: true)
