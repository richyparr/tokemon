...
struct UsageHeaderView: View {
    ...
    private var headerColor: Color {
        if usage.source == .none {
            return themeColors.tertiaryText
        }
        if usage.hasPercentage {
            // Use theme-aware usage color (handles Light theme for low percentages)
            return themeColors.usageColor(for: usage.primaryPercentage)
        }
