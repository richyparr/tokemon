...
struct JSONLParser {
    ...
    /// Each directory represents a Claude Code project (e.g., `-Users-richardparr-Tokemon`).
    ...
    static func parseSession(at url: URL) -> SessionUsage {
        ...
        if skippedLines > 0 {
            print("[Tokemon] JSONL parser: skipped \(skippedLines) malformed lines in \(url.lastPathComponent)")
        }
